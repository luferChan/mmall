...
public class UserController {
    ...
    @ResponseBody
    public ServerResponse<User> update_information(User user,HttpServletRequest httpServletRequest){
        String loginToken = CookieUtil.readLoginToken(httpServletRequest);
        if (StringUtils.isEmpty(loginToken)){
            return ServerResponse.createByErrorMessage("用户未登录,无法获取当前用户的信息");
        }
        ...
        if(response.isSuccess()){
            ...
            //session.setAttribute(Const.CURRENT_USER,response.getData());
            RedisShardedPoolUtil.setEx(loginToken,Const.RedisCacheExTime.REDIS_SESSION_TIME, JsonUtil.obj2String(response.getData()));
        }
        return response;
    }
